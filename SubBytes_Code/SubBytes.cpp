...
// Bảng Inverse S-box: Dùng để giải mã ngược về trạng thái cũ
...
void invSubBytes(uint32_t state[4]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        uint8_t b0 = rsbox[(word >> 24) & 0xFF];
        uint8_t b1 = rsbox[(word >> 16) & 0xFF];
        uint8_t b2 = rsbox[(word >> 8)  & 0xFF];
        uint8_t b3 = rsbox[word         & 0xFF];
